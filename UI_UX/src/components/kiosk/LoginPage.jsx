...
export default function LoginPage() {
  ...
  return (
    <div className="screen bg-koisk-surface">
      {/* Top bar — FIX (High): logout/back controls now meet min touch target + contrast */}
      <div className="bg-koisk-navy px-6 py-4 flex items-center justify-between">
        ...
        <button
          onClick={() => navigate('/')}
          className="text-white hover:text-koisk-accent font-body text-sm transition-colors min-h-[44px] min-w-[44px] focus-visible:outline focus-visible:outline-2 focus-visible:outline-koisk-accent rounded-lg px-2"
        >
          {t('language.change')}
        </button>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10">
        <div className="w-full max-w-md animate-fade-up">
          ...
          {/* Form — FIX (High): every label now has htmlFor matching its input's id */}
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="login-phone" className="input-label">
                {t('auth.phone')}
              </label>
              <input
                id="login-phone"
          ...
          <button
            onClick={handleDemo}
            disabled={loading}
            className="w-full p-4 rounded-2xl border-2 border-dashed border-koisk-teal/40 text-koisk-teal font-display font-semibold text-base hover:bg-koisk-teal/5 transition-all active:scale-95 disabled:opacity-40 focus-visible:outline focus-visible:outline-2 focus-visible:outline-koisk-teal"
          >
          ...
          <p className="text-center mt-6 text-koisk-muted font-body">
            {t('auth.no_account')}{' '}
            <Link to="/register" className="text-koisk-teal font-semibold hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-koisk-teal rounded">
